...
class Sign(Base):
    ...
    def get_info(self):
        ...
        self._nickname_list = [(i.get('nickname', 'NA')) for i in role_list]

        log.info('准备获取签到信息...')
        for i in range(len(self._uid_list)):
            info_url = CONFIG.INFO_URL.format(CONFIG.ACT_ID, self._uid_list[i], self._region_list[i])
            try:
                content = requests.Session().get(
    ...
    def run(self):
        ...
        for i in range(len(info_list)):
            ...
            nickname = self._nickname_list[i]

            log.info(f'准备为角色 {i + 1} 号签到...')
            time.sleep(2)
            messgae = {
                'today': today,
